...
(define (retag-THeap τ expect theap-tag)
  (if (THeap? expect)
      (match expect
        [(THeap: sy taddr tag σ)
         (*THeap 'retag-THeap sy taddr (or tag theap-tag) σ)]
        [_ τ])
      τ))
        ...
        [(THeap? shape-τ)
         (match (resolve (type-meet (heapify-generic expected) shape-τ))
           [(THeap: sy taddr tag* τ)
            (values 'down sy taddr tag*
                    (λ (constr) (direct-construction 'down sy taddr (or tag* tag) constr))
                    values
                    τ)]
           [_ (values #f #f #f #f values values T⊥)])]
        ;; Upcast
        [else
         (match (resolve (type-meet expected (heapify-generic shape-τ)))
           [(THeap: sy taddr tag* τ)
            (values 'up sy taddr tag*
                    (λ (constr) (direct-construction 'up sy taddr (or tag* tag) constr))
                    (λ (τ) (mk-THeap sy taddr tag* τ))
                     τ)]
           [_ (values #f #f #f #f values values T⊥)])])]
      [else (values #f #f #f #f values values non-H)])))


(define (tc-pattern Γ Ξ pat expect)
  (define (tc Γ pat expect)
    (define (assert-up Hdir)
      (when (and Hdir (not (eq? Hdir 'up)))
        (error 'tc-pattern "Broken invariant: patterns should always upcast: ~a" pat)))
    ...
    (match pat
      ...
      [(PName sy _ x)
       (match (hash-ref Γ x #f)
         [#f
          (define ct (chk expect))
          (values (hash-set Γ x (πct ct)) (PName sy ct x))]
         [τ
      ...
      [(PVariant sy _ n ps)
       ...
       (define-values (Hdir Tsy taddr tag W TW τ) (reshape (generic-variant n len)))
       (assert-up Hdir)
       ...
       (let all ([Γ Γ] [ps ps] [exs expects] [τs-rev '()] [rev-ps* '()])
         (match* (ps exs)
           [('() '())
            ...
            (define mkp (if Hdir (λ (ct) (PDeref Tsy (mkp0 ct) #t)) mkp0))
            (values Γ (mkp (or err (chk (mk (reverse τs-rev))))))]
       ...
       [(or (and (PMap-with sy _ k v p) (app (λ _ PMap-with) ctor))
        ...
        (define-values (Hdir Tsy taddr tag W TW τ) (reshape generic-map))
        (assert-up Hdir)
        ...
        (values Γ***
                (mkp (or err
                         (chk (type-join (mk (πcc k*) (πcc v*)) (πcc p*))))))]

       [(or (and (PSet-with sy _ v p) (app (λ _ PSet-with) ctor))
            (and (PSet-with* sy _ v p) (app (λ _ PSet-with*) ctor)))
        (define-values (Hdir Tsy taddr tag W TW τ) (reshape generic-set))
        (assert-up Hdir)
        ...
        (define mkp (if Hdir (λ (ct) (PDeref Tsy (mkp0 ct) #t)) mkp0))
        (values Γ**
                (mkp (or err (chk (type-join (mk (πcc v*)) (πcc p*))))))]
       ...
       [(PDeref sy _ p imp)
        ...
        (cond
         [imp
          (cond
           [(check-for-heapification?)
            (define ex* (type-meet expect (heapify-generic T⊤)))
            (define err
              (and (T⊥? ex*) (type-error "Implicit deref expected heapified type, got ~a" expect)))
            (define-values (Γ* p*) (tc Γ p (if err T⊤ ex*)))
            (values Γ* (PDeref sy (or err (Cast (πcc p*))) p* imp))]
           [else
            (define-values (Γ* p*) (tc Γ p expect))
            (values Γ* (PDeref sy (Typed-ct p*) p* imp))])]
         [else
          (define ex* (type-meet expect generic-TAddr))
          (define err
            (and (T⊥? ex*) (type-error "Explicit deref expected an address type, got ~a" expect)))
          (define-values (Γ* p*) (tc Γ p (if err T⊤ ex*)))
          (values Γ* (PDeref sy (Cast (πcc p*)) p* imp))])]

       [(PTerm sy _ t)
        (define t* (tc-term Γ Ξ t expect))
        (values Γ (PTerm sy (chk (Typed-ct t*)) t*))]
...
(define (direct-construction Hdir sy taddr tag constr)
  (case Hdir
    [(down)
     (λ (ct)
        (define e (constr ct))
  ...
  (define (tc-expr* e expected path #:tagged [tagged #f])
    ...
    (match e
      ...
      [(EVariant sy _ n tag τs es)
       ...
       (define-values (Hdir Tsy taddr tag* W TW τ) (reshape generic (or tag tagged path)))
       ...
       (define (do σs tr)
         ...
         ((W mkp0) (chk (TW (mk-TVariant #f n (map πcc es*) tr)))))
       (define (infer)
      ...
      [(ERef sy _ x)
       (define-values (Hdir Tsy taddr tag W TW τ)
         (reshape (hash-ref Γ x (unbound-pat-var sy 'tc-expr x)) (or tagged path)))
       (define mkp0 (λ (ct) (ERef sy ct x)))
       ((W mkp0) (chk (TW τ)))]
      ...
      [(EAlloc sy _ tag)
       (EAlloc sy (chk generic-TAddr) (give-tag tag path))]
      ...
      [(EExtend sy _ m tag k v)
       ...
       (define mkp0 (λ (ct) (EExtend sy ct m* (give-tag tag* path) k* v*)))
       ((W mkp0) (chk (TW (type-join (πcc m*) (W (mk-TMap #f (πcc k*) (πcc v*) ext))))))]

      [(EEmpty-Map sy _)
       (define-values (H? Tsy taddr tag W TW τ) (reshape generic-map (or tagged path)))
       (define mkp0 (λ (ct) (EEmpty-Map sy ct)))
       ((W mkp0) (chk (TW τ)))]

      [(EEmpty-Set sy _)
       (define-values (H? Tsy taddr tag W TW τ) (reshape generic-set (or tagged path)))
       (define mkp0 (λ (ct) (EEmpty-Set sy ct)))
       ((W mkp0) (chk (TW τ)))]
...
(define (report-pattern-errors pat)
  ...
  (match pat
    ...
    [(or (PSet-with _ _ v p)
     ...
     (report-pattern-errors v)
     (report-pattern-errors p)]
    [(PDeref _ _ p _) (report-pattern-errors p)]
